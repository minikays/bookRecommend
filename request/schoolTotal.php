<?php
	...
	if ($count != 0)
	{
		...
		$totaldays = 0;
		//游历国家
		$sql = "SELECT place,days from tour where fillYear = '$fillYear' and department='$department' ";
		$result = mysql_query($sql,$con);
		$ret = array();
		$r = array();
		$count = 0;
		...
		//参与游历国家总人数；
		$sql="SELECT COUNT(distinct(sid)) AS count FROM tour WHERE fillYear = '$fillYear' and department='$department'"; 
		$result=mysql_fetch_array(mysql_query($sql)); 
		$count=$result['count']; 
		$arr["cntTourNum"] = "$count";
		
		//参与游历国家总次数；
		$sql="SELECT COUNT(*) AS count FROM tour WHERE fillYear = '$fillYear' and department='$department'"; 
		$result=mysql_fetch_array(mysql_query($sql)); 
		$count=$result['count'];
